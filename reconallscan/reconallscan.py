...
def mssqlEnum(ip_address, port):
    ...
    MSSQLSCAN = "nmap -vv -sV -Pn -p %s --script=ms-sql-info,ms-sql-config,ms-sql-dump-hashes --script-args=mssql.instance-port=1433,smsql.username-sa,mssql.password-sa -oX /root/Desktop/results/exam/nmap/%s_mssql.xml %s" % (port, ip_address, ip_address)
...
if __name__=='__main__':
   f = open('/root/Desktop/results/exam/targets.txt', 'r') # CHANGE THIS!! grab the alive hosts from the discovery scan for enum
   for scanip in f:
       jobs = []
